...
const accountSession = window.UrbanWearAuth.getSession();
let activeDrawerOrder = null;
let accountProfile = accountSession ? window.UrbanWearProfile.getProfile(accountSession) : null;
...
function renderAccountIdentity() {
  const firstName = accountProfile.firstName || accountSession.name.split(" ")[0];
  ...
  document.getElementById("accountGreeting").textContent = `Вітаємо, ${firstName}! Раді вас бачити знову.`;
}
...
function renderAccountCart() {
  ...
  target.innerHTML = `<div class="customer-cart-table-wrap"><table class="customer-cart-table customer-cart-table--premium"><thead><tr><th>Товар</th><th>Розмір</th><th>Колір</th><th>Ціна</th><th>Кількість</th><th>Сума</th><th>Дія</th></tr></thead><tbody>${rows.map((row) => `<tr>
    ...
    <td><strong>${accountPrice(parsePrice(row.product.price) * row.quantity)}</strong></td><td><button type="button" data-account-cart-remove="${escapeAccountHtml(row.id)}" aria-label="Видалити ${escapeAccountHtml(row.product.title)}">⌫</button></td></tr>`).join("")}</tbody></table></div>
    <div class="customer-cart-bottom">
      ...
      <form class="customer-promo" id="accountPromoForm"><label>Промокод<input name="promo" value="${escapeAccountHtml(promo)}" placeholder="Введіть промокод" /></label><button type="submit">Застосувати</button><p id="accountPromoMessage"></p></form>
...
function renderFavorites() {
  ...
  target.innerHTML = `<div class="customer-favorites-grid">${favorites.map((id) => {
    ...
    return `<article class="customer-favorite-product"><div class="customer-favorite-image" style="${productImageStyle(product)}"><button type="button" data-favorite-product="${escapeAccountHtml(id)}" aria-label="Видалити з обраного">♥</button><span>${escapeAccountHtml(product.imageLabel)}</span></div><div class="customer-favorite-body"><h2>${escapeAccountHtml(product.title)}</h2><small>SKU: ${escapeAccountHtml(id)}</small><div class="customer-favorite-swatches"><i style="background:${productColorValue(product)}"></i><i></i><i></i></div><strong>${escapeAccountHtml(product.price)}</strong><button class="btn btn--dark" type="button" data-add-to-cart="${escapeAccountHtml(id)}" data-default-text="Додати в кошик">Додати в кошик</button><a class="btn btn--light" href="product.html?id=${encodeURIComponent(id)}">Переглянути товар</a></div></article>`;
  }).join("")}</div>`;
...
function logoutAccount() { window.UrbanWearAuth.logout(); window.location.href = "index.html"; }

if (!accountSession || accountSession.role !== "customer") {
...
if (requestedTab && document.querySelector(`[data-account-panel="${CSS.escape(requestedTab)}"]`)) activateAccountTab(requestedTab);
